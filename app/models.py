...
class Question(models.Model):
    ...
    score = models.ManyToManyField(User, through="app.QuestionLike", related_name="questionliked")

    objects = QuestionManager()

    def get_likes(self):
        return QuestionLike.objects.filter(type=True, question=self).count() - QuestionLike.objects.filter(type=False, question=self).count()
...
class Answer(models.Model):
    ...
    score = models.ManyToManyField(User, through="app.AnswerLike", related_name="answerliked")

    objects = AnswerManager() 

    def get_likes(self):
        return AnswerLike.objects.filter(type=True, answer=self).count() - AnswerLike.objects.filter(type=False, answer=self).count()
